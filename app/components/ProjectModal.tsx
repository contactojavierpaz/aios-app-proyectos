...
export function ProjectModal({ project, onSave, onClose }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!name.trim()) return;
    onSave({ name: name.trim(), status, nextStep: nextStep.trim(), quickNote: quickNote.trim() });
    onClose();
  }
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-6 space-y-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              className="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
              placeholder="¿Qué necesitas hacer ahora?"
              value={nextStep}
              onChange={e => setNextStep(e.target.value)}
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Nota rápida</label>
            <textarea
              ...
              placeholder="Contexto, bloqueos, ideas..."
              rows={3}
              value={quickNote}
